        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
          {[
            ...
            <div key={unit.label} className="flex flex-col items-center">
              <motion.div
                key={unit.value} // triggers re-animation on value change
                initial={{ scale: 1.1 }}
                animate={{ scale: 1 }}
                transition={{ type: "spring", stiffness: 300, damping: 15 }}
                className="w-20 h-24 sm:w-28 sm:h-32 bg-[#f2e1b3] rounded-2xl border-4 border-[#000650] shadow-[6px_6px_0px_#000650] flex items-center justify-center overflow-hidden relative"
              >
                {/* Glossy highlight */}
                <div className="absolute top-0 left-0 w-full h-1/2 bg-white/30" />
                <span className="text-4xl sm:text-6xl font-poppins font-black text-[#000650] relative z-10">
                  {unit.value.toString().padStart(2, "0")}
                </span>
              </motion.div>
              <span className="mt-4 font-poppins font-black text-white text-sm sm:text-base tracking-widest">
                {unit.label}
              </span>
            </div>
          ))}
